...
GROUND_TRUTH="${EXTRACTION}/golden/kernel_cancel_case2/ground_truth.json"

BC="${1:-/tmp/io_uring_cancel.bc}"
OUT="${2:-/tmp/io_uring_cancel.facts.jsonl}"
...
if [[ ! -f "${BC}" ]]; then
  ...
  echo "hint: generate from linux-6.1 io_uring/cancel.c (see extraction/golden/kernel_cancel_case2/README.md)" >&2
  exit 1
fi
...
python3 "${EXTRACTION}/scripts/check_kernel_cancel_case2.py" \
  "${OUT}" \
  "${GROUND_TRUTH}"
