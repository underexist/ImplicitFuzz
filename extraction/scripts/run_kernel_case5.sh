#!/usr/bin/env bash
# Kernel case5: io_op_defs const dispatch table -> entry_fact (opcode->handler).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
EXTRACTION="${ROOT}/extraction"
BUILD_DIR="${EXTRACTION}/build"
GROUND_TRUTH="${EXTRACTION}/golden/kernel_opdef_dispatch_case5/ground_truth.json"

BC="${1:-/tmp/io_uring_opdef.bc}"
OUT="${2:-/tmp/io_uring_opdef.facts.jsonl}"
...
if [[ ! -f "${BC}" ]]; then
  ...
  echo "hint: generate from io_uring/opdef.c (same flags as other kernel cases)" >&2
  exit 1
fi
...
python3 "${EXTRACTION}/scripts/check_kernel_opdef_case5.py" \
  "${OUT}" \
  "${GROUND_TRUTH}"
