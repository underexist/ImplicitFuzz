...
17. **[done] entry_fact via const dispatch-table reader** — read `io_op_defs` constant initializer in opdef.c → 49 opcodes → prep/issue/cleanup/prep_async/fail handlers (137 entry_fact `op_dispatch` rows; roles from DWARF by byte offset). Resolves the opcode dispatch (an unresolvable per-TU indirect call, since `io_op_defs` is `external[0x]` in io_uring.c) and populates the previously-empty entry_fact / opcode→handler attribution — the prerequisite for syscall-centric evidence. Kernel case5 golden. Plan: `docs/superpowers/plans/2026-07-10-opcode-entry-and-branch-facts.md`; reuse rationale: `docs/related-work-static-reuse.md`.
18. **[done] branch_fact + gate_seed_candidate (state-gate static skeleton)** — branch_fact ties each conditional/switch branch to the object fields it loads (bounded branch-local backward slice); gate_seed_candidate marks branches gated on written (state-carrier) fields. 4-TU smoke: 65 seeds / 26 functions on real io_uring state fields (io_kiocb.flags/link, io_timeout.head, io_cancel.flags, ...). Static half of 研究内容一's state-machine view; LLM predicate inference is the deliberate next boundary. See `docs/phase2d-state-gates.md`.
19. [next] 反查 + LLM: join gate_seed_candidate fields → write access_facts (prior calls) across the corpus; LLM infers predicates (premise/activation/param-align) under code slices with 3-gate hallucination validation
19. [later] evidence-DB join: attribute io_uring.c's unresolved dispatch `call_fact` to `entry_fact` candidate sets by `dispatch_table` (declarative join, keeps extraction per-TU)
20. [later] kernel wrapper propagation — only if audit finds direct primitive patterns in TU bitcode
21. [later] BTF in provenance / main recovery chain
22. [later] container_of / list_entry
23. [later] unify wrapper-call-site vs. Andersen points-to granularity (see phase2c doc's "Known, accepted limitation")
24. [later] general whole-program indirect-call resolution for residual heap-stored callbacks (io_wq/poll/task_work) — SoTA is KallGraph; see `docs/related-work-static-reuse.md` (conditional-future, needs whole-kernel bitcode)
```
